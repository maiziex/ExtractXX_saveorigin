...
def read_qname_barcode_dict(qname_barcode_txt_file,qname_pickle):
    f = open(qname_barcode_txt_file,"r")
    qname_barcode_dict = defaultdict()
    curr = 0
    for line in f:
        print(curr)
        curr += 1
        ...
        qname = data[0]
        barcode = data[1]
        qname_barcode_dict[qname] = barcode
    pickle.dump(qname_barcode_dict,open(qname_pickle,"wb"))
            ...
            for key, value in mole_qname.items():
                ...
                if choose_num > 1:
                    ...
                    if total_qname_num > 0:
                        #print(str(float(choose_num)/total_qname_num))
                        print("xin" + str(count_mole))
                        if total_qname_num >= choose_num:
                            choose_qname_list = random.sample(value.keys(), choose_num)
                        else:
